        ...
        for i in range(1,113):
            ...
            with open(arg, 'r') as f:
                ...
                for name, n in count.items():
                    ...
                    line = str(i) + ',' + str(avg_pow * avg_time) + ',' + str(avg_time) + ',' + str(avg_pow) + ',' + str(speedup)
                    #line = str(i) + "," + str(avg_pow) + "," + str(max_pow) + "," + str(avg_time) + "\n"
                    results[name] = results.get(name,{})
                    results[name][i] = line

        for name,res in results.items():
            with open(name + '.' + str(freq) + ".csv", 'a') as resfile:
                resfile.write('nthreads,energy,time,power,speedup\n')
                for s in sorted(res):
                    resfile.write(res[s] + '\n')
